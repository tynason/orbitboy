...
class Mandel(object):
	...
	def __init__(self,doani,doloop,dosave,doang,dodata,chunk,chunksleep,maxiters,minbored,boreme,maxrad,trimend,numbins,numgrads,figclose,figsleep,finalsleep,linesleep,wid,ht,xpos,ypos):
		self.doani=doani
		self.doloop=doloop
		self.dosave=dosave
		self.doang=doang
		...
		for n in range(0,k-2):
			...
			if self.doang:
				for n in range(0,k-2):
					...
					if abs(magA)<1e-8:
						angerr='magA=0'
						print('EXCEPTION angerror: ', angerr); break
					if abs(magB)<1e-8:
						angerr='magB=0'
						print('EXCEPTION angerror: ', angerr); break
	...
	def plotme(self):
		...
		while True:
			...
			myheader = ['kappa','p','q','maxrad','maxiters','radavg','raddev','angavg','angdev','angerr']
			for item in myheader: print('{: <22}'.format(item),end='')
			print('\n')
			mydata = [kappa,p,q,self.maxrad,self.maxiters,radavg,raddev,angavg,angdev,angerr]
			for item in mydata: print('{: <22}'.format(item),end='')
			print('\n')
			...
			# plot some stuff
			mytitle='Mandelbrot ' +self.mandfunc+ '  p= '+str(repr(p))+'  q= '+str(repr(q))+'  kappa= '+str(kappa)+'  maxrad= '+str(self.maxrad)
			...
			#print(p,q,kappa,maxrad,radavg,raddev,angavg,angdev)
			#print(type(p),type(q),type(kappa),type(maxrad),type(radavg),type(raddev),type(angavg),type(angdev))
			#<class 'float'> <class 'float'> <class 'int'> <class 'float'> <class 'numpy.float64'> <class 'numpy.float64'> <class 'numpy.float64'> <class 'numpy.float64'>

			nice.append(params);xnice.append(params[1]);ynice.append(params[2])
			intfile=str(kappa)+'_'+str(repr(p))+'_'+str(repr(q))+'.txt'

			if self.dosave: 

				#make a folder called imgs if it does not already exist
				if not os.path.isdir('imgs'): os.mkdir('imgs') 

				# save png
				outfile=str(kappa)+'_'+str(repr(p))+'_'+str(repr(q))
				...
				try:
					...
					cursor = cnx.cursor()
					add_orbit = 'INSERT INTO orbit (p,q,kappa,maxrad,radavg,raddev,angavg,angdev) VALUES ('+str(repr(p))+','+str(repr(q))+','+str(kappa)+','+str(self.maxrad)+','+str(radavg)+','+str(raddev)+','+str(angavg)+','+str(angdev)+')'
					#print("add_orbit")
					#print(add_orbit)
					...
					cnx.close()
					# test 
					# INSERT INTO orbitt (p,q,kappa,maxrad,radavg,raddev,angavg,angdev) VALUES('-0.7449928513059887','0.08224038199273027','2474','2000.0','0.331238360676','0.27409997834','174.98285257','28.6254725733');
					#print(add_orbit)

				# if no DB or bad credentials, save text file with orbit parameters
				except:
					intfile='imgs/'+intfile
					intdata=''
					for el in nice:
				...
				if dopoints:
					try:
						conn = mysql.connector.connect(user='root', password='YOURMYSQLPASSWORD', host='127.0.0.1', database='mandel')
						cursor = conn.cursor()

						orb_idd=cursor.execute("SELECT @last_id := MAX(orbit_id) FROM orbit")
						result = cursor.fetchone()
						orb_id=result[0]
						#print("orb_id"); print(orb_id)					

						zdata=zip(xdata,ydata)
						for el in zdata:
							add_point = 'INSERT INTO points (orbit_id,kappa,x,y) VALUES ('+str(orb_id)+','+str(kappa+1)+','+str(el[0])+','+str(el[1])+')'
							#print("add_point"); print(add_point)
							cursor.execute(add_point)

							# alt syntax
							#query = "INSERT INTO points (orbit_id, kappa, x ,y) VALUES (%s, %s, %s, %s)"
							#val = ("2649","1545",0.077777777777","0.9999999999")
							#cursor.execute(query, val)	

							# test
							# INSERT INTO points (orbit_id,x,y) VALUES (2651,0.077777777777,0.9999999999)

							# export to csv
							#SELECT points_id,x,y FROM points WHERE orbit_id = '2662' INTO OUTFILE '/var/lib/mysql-files/orbit01.csv' FIELDS TERMINATED BY ',' ENCLOSED BY '"'LINES TERMINATED BY '\n';
							#sudo mv /var/lib/mysql-files/orbit01.csv ~/Desktop

						conn.commit()

					except:
						print('POINTS INSERT FAILED')
...
wid=1800;ht=1300;xpos=0;ypos=0

mymand=Mandel(doani,doloop,dosave,doang,dodata,chunk,chunksleep,maxiters,minbored,boreme,maxrad,trimend,numbins,numgrads,figclose,figsleep,finalsleep,linesleep,wid,ht,xpos,ypos)
